...
const BlogIndexLayout = ({
  ...
  const filteredPosts = posts.filter((post) => {
    ...
    const matchesCategory =
      !selectedCategory ||
      post.primaryCategory?.slug.current === selectedCategory
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#0F0E13' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* Trending Articles from Settings */}
        {settings?.trendingArticles && settings.trendingArticles.length > 0 && (
          <div className="mt-16 border-t border-gray-800 pt-12">
            <h2 className="text-3xl text-white mb-8">
              Trending Articles
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {settings.trendingArticles.slice(0, 3).map((post: Post) => (
                <BlogCard
                  key={post._id}
                  _key={post._id}
                  title={post.title}
