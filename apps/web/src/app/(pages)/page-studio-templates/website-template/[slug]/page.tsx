...
export async function generateStaticParams() {
  // Obtener solo templates de Site sin Mandrel ID válido
  const siteTemplates = await getStaticPathsParams({
    filter: `kind == "${TemplateKind.Site}" && (!defined(id) || id == null || id == '' || id match "*example*" || id match "*test*" || id match "*ejemplo*" || id match "*prueba*" || length(id) < 10) && !defined(lastMandrelUpdate) && !defined(originalCreatedAt)`,
    types,
  })

  const paths = siteTemplates.map(({ params: { slug } }: any) => ({
    slug,
  }))
...
export default async function Page({
  ...
  // Fetch template data (Site templates without Mandrel ID)
