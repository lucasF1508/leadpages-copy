...
const getProductSlug = (_slug: string[] | string) => {
  const slug = Array.isArray(_slug) ? _slug : [_slug]
  return ['product', ...slug].filter(Boolean).join('/')
}
...
export async function generateMetadata(
  ...
  const path = ['/', getProductSlug(slug)].join('')
...
export default async function Page({
  ...
  const { slug = [] } = await params
  const {components, hero} = await pageQuery(getProductSlug(slug)).data || {}
...
export async function generateStaticParams() {
  const params = await getStaticPathsParams({ types: ['page'], filter: `path match '/product*'`, catchAll: 'product' })
