...
export const getStaticProps = async (context) => {
  ...
  const {slug} = params || {}
  return getGenericStaticProps(`/conversion-optimization-guide/${slug}`, preview)
}

export const getStaticPaths = async () => {
  const paths = await getStaticPathsParams({type: 'page', filter: 'path match "/conversion-optimization-guide/*"'})
  return {
    paths,
    fallback: false,
  }
