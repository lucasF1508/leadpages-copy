...
export function getExpiredAndInvalidate(storageKey, browserExpirationDate, apiExpirationDate) {
  let expired = false;
  if (apiExpirationDate || browserExpirationDate) {
    // Parse UTC string
    const parsedApiExpiration = apiExpirationDate && Date.parse(apiExpirationDate);
    // Browser expiration will already be a Unix Epoch.
    const parsedBrowserExpiration = browserExpirationDate && browserExpirationDate;

    const currentTime = new Date().getTime();

    const isApiDateExpired = parsedApiExpiration ? parsedApiExpiration < currentTime : false;
