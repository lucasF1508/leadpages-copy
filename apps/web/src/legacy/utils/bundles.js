...
async function fetchBundle(bundleId, flow) {
  ...
  if (
    ...
    (flow === FLOWS.REACTIVATION && !bundleResponse.details?.enabledForReactivation)
  ) {
    return { error: { message: OFFER_INVALID_MESSAGE } };
  }
...
export function checkBundleExpired(bundle) {
  ...
  return getExpiredAndInvalidate(BUNDLE_KEY, bundle?.browserExpiration, bundle?.expirationDate);
}
...
export async function setLocalBundle(flow) {
  ...
  if (bundleParamId) {
    ...
    if (typeof window !== 'undefined' && bundle && !bundle.enabled) {
      ...
      return bundle.error ? bundle : { error: { message: OFFER_EXPIRED_MESSAGE } };
    }
  }
