...
import { checkCouponExpired, getLocalCoupon, setLocalCoupon } from '../../utils/coupons';
...
const Promotions = ({ onPromotionsLoaded }) => {
  ...
  useEffect(() => {
    async function promotionsInit() {
      ...
      const [couponResponse, bundleResponse] = await Promise.all([couponPromise, bundlePromise]);
      ...
      const localCoupon =
        couponResponse && !couponResponse.error ? couponResponse : getLocalCoupon();
  ...
  return (
    <>
      {showBundle && bundle.details?.infoSheet?.enabled && (
        <BundleInfoSheet
          backgroundColor={bundle.details.infoSheet.backgroundColor || undefined}
      ...
      {showCoupon && (
        <Banner
          ...
          selector=".banner"
          CTA={!onPricingPage ? <Button href="/pricing">Get This Deal</Button> : null}
        />
      )}
