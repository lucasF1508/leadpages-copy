...
const CardsComparison = ({ comparisons }) => (
  <OuterContainer>
    {comparisons.map(({ _key, title, excerpt, slug }) => {
      const { compareLogo, content } = excerpt || {}
      const href = `comparisons/${slug?.current}`
          ...
          <Link href={href} passHref legacyBehavior>
            <StyledLink aria-label={title}>
              <CardImageItem>
                <VersusContainer>
                  <LeadpagesSVG
                    src={leadpagesLogoSVG.src}
                    alt="Leadpages logo"
                  />
                  <VersusText>vs.</VersusText>
                </VersusContainer>
                <CompetitorSVG
                  src={compareLogo.asset.url}
                  alt={compareLogo.altText || 'Competitor Logo'}
                />
              </CardImageItem>
              <TextContentItem>
