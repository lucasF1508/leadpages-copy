...
const shapeData = (data, currentDocs = []) => {
  const currentDocIds = new Set(
    currentDocs.map((doc) => doc._id.replace(/^drafts\./, ''))
  )
  ...
  const { updates, newDocs } = data.reduce(
    (acc, { _meta, kind, template }) => {
      ...
      if (currentDocIds.has(id)) {
        const { _updatedAt } = currentDocs.find(
          (doc) => doc._id.replace(/^drafts\./, '') === id
        )

        const requiresUpdate =
          _updatedAt &&
          _meta?.updated &&
          new Date(_updatedAt) <= new Date(_meta.updated)
  ...
  // For testing: deletes all documents
  // const deletes = currentDocs.map(({ _id }) => ({
  //   _id,
  // }))
...
const templatesListItems = async () => {
  ...
  const {
    ...
    updates: _updates,
    // deletes,
  } = shapeData(mandrelTemplates, sanityTemplateDocs)

  const [newDocs, updates] = await Promise.all([
    Promise.all(_newDocs),
    Promise.all(_updates),
  ])

  // const deletePatches = buildDeletePatches(deletes)
