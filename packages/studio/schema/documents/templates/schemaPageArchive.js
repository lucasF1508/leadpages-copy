...
const pageTemplates = ['post', 'customer', 'integration', 'comparison', 'feature', 'faq']
...
export const schemaPageArchive = {
  ...
  fields: [
    ...F.fieldDefaultsCustom({
      ...
      parent: {
        readOnly: true,
        hidden: ({parent}) => !['feature-index'].includes(parent?.slug?.current),
      },
    }),
    ...G.group('content', [
      F.field('hero', {
    ...
    ...G.group('options', [
      F.boolean({
        ...
        hidden: ({parent}) =>
          !['integrations', 'comparisons', 'feature-index'].includes(parent?.slug?.current),
      }),
      F.object({
        name: 'options',
        fields: [
