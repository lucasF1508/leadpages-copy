...
const excludedKeys = ['_createdAt', '_id', '_rev', '_type', '_updatedAt', 'slug', 'path']
...
const OverrideContent = ({id, type, onComplete} = {}) => {
  ...
  const formatOptions = (docs) => docs.map(({title, _id}) => ({label: title, value: _id}))

  const getOptions = async () => {
    const documents = await client.fetch(`*[_type == $type] | order(_updatedAt desc)`, {
      type,
    })
  ...
  const handleUpdate = async () => {
    ...
    const set = filteredKeys.reduce((obj, key) => ({...obj, [key]: selected[key]}), {})
