import parseLinks from "@src/utils/parseLinks"
import plainTextToPortable from "@src/utils/plainTextToPortable"
import { omit } from "lodash"

const shapeGlobalCta = (component: Record<string, any>): Record<string, any> => {
  const filteredFields = omit(component, ["bgColor", "overline", "title"])

  return {
    ...filteredFields,
    backgroundImage: component.backgroundImage || {
      _type: 'image',
      asset: {
        _ref:"image-afaed729d30bffe803801a5f356929095e0c50f8-669x516-png",
        _type:"reference"
      },
      lqip:"data:image/png;base64,"
    },
    content: [...plainTextToPortable(component.overline, 'small'), ...component.content],
    desktopOrientation: 'horizontal',
    heading: component.title,
    links: parseLinks(component.links)
  }
}

export default shapeGlobalCta