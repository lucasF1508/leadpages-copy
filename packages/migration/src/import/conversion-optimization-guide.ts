...
const mapGuide = async (docs) =>
  docs.map((doc) => {
    const sections = doc?.sidebarLinks?.map((section) => ({
      _key: uuid(),
      _type: 'sidebarSection',
      heading: section.title,
      links: section.links.map(({ link }) => ({
        _key: uuid(),
        _type: 'link',
        condition: 'internal',
        label: link?.page?.title || 'Conversion Optimization Guide',
        url: link?.page?.path || '/conversion-optimization-guide',
      })),
    }))
      ...
      components: [
        {
          ...
          content,
          sidebar: {
            heading: 'Conversion Optimization Guide',
            sections,
          },
        },
        ...(next
...
const migrateDocs = async () => {
  const data =
    await from.fetch(`*[path match '/conversion-optimization-guide*']{
    ...,
    sidebarLinks[]{
      ...,
      links[]{
